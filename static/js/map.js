...
let init = (app) => {
  ...
  app.resetMarkerSize = function (marker) {
    if (app.vue.markerSelected.length != 0) {
      //curr marker selected
    }
    if (marker === null) app.vue.markerSelected.pop();
  ...
  app.search = function () {
    if (app.vue.query.length > 1) {
      axios
        .get(searchURL, { params: { q: app.vue.query } })
    ...
    } else app.vue.results = [];
  };

  app.redirectCacheInfo = function () {
    axios
      .get(generateCacheURL, { params: { cache_id: app.vue.cacheID } })
      .then(function (r) {
  ...
  app.setPopup = function () {
    if (app.vue.popupMode == true) {
      //sounds redundant, but for transition of popups
      app.vue.popupMode = false;
      setTimeout(() => {
        app.vue.popupMode = true;
      }, '200');
    } else app.vue.popupMode = true;
  };
...
function setupMap(center) {
  ...
  map.on('click', () => {
    //Click event to remove/toggle popup and size for marker
    app.vue.query = '';
  ...
  var loaded = false;
  setTimeout(() => {
    //Ensures loading screen is displayed for a minimum time
